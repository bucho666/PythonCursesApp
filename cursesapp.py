...
class CursesScreen(object):
  ...
  def initialize(self):
    self._screen = CursesScreenBuilder.build()
    self._color_table = Color.table()
    return self
...
class CursesScreenBuilder(object):
  @classmethod
  def build(cls):
    Color.initialize()
...
class CursesApplication(object):
  ...
  def _main(self, args):
    self._screen = CursesScreen().initialize()
    self.initialize()
    while True:
      self.process(self._screen.read_key())
...
if __name__ == '__main__':
  class Demo(CursesApplication):
    import sys
    def __init__(self):
      CursesApplication.__init__(self) 
